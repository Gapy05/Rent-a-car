...
public class uredi extends JFrame implements ActionListener {
    private Container c;
    ...
    private JButton naloziteSlikoButton;
    private File selectedFile;
    private int voziloId;

    public uredi(int voziloId) {
        this.voziloId = voziloId;
        setTitle("Urejanje avtomobila");
        ...
        title = new JLabel("Urejanje avtomobila");
        ...
        try {
            ...
            String sql = "SELECT Registracija, Cena, model_ID, barva_ID FROM Vozilo WHERE ID = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setInt(1, voziloId);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
                tregistracija.setText(rs.getString("Registracija"));
                tcena.setText(rs.getString("Cena"));

                int modelId = rs.getInt("model_ID");
                int barvaId = rs.getInt("barva_ID");

                // Get model name
                sql = "SELECT ime FROM Model WHERE ID = ?";
                preparedStatement = connection.prepareStatement(sql);
                preparedStatement.setInt(1, modelId);
                ResultSet modelRS = preparedStatement.executeQuery();
                if (modelRS.next()) {
                    tmodel.setSelectedItem(modelRS.getString("ime"));
                }

                // Get barva name
                sql = "SELECT ime FROM Barva WHERE ID = ?";
                preparedStatement = connection.prepareStatement(sql);
                preparedStatement.setInt(1, barvaId);
                ResultSet barvaRS = preparedStatement.executeQuery();
                if (barvaRS.next()) {
                    tbarva.setSelectedItem(barvaRS.getString("ime"));
                }
            }
        ...
        sub = new JButton("Uredi avtomobil");
        ...
        naloziteSlikoButton.addActionListener(this);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == sub) {
            ...
            try {
                ...
                String sql = "UPDATE Vozilo SET Registracija = ?, Cena = ?, model_ID = ?, barva_ID = ?, slika = ? WHERE ID = ?";
                ...
                if (selectedFile != null) {
                    statement.setBytes(5, Files.readAllBytes(selectedFile.toPath()));
                } else {
                    statement.setNull(5, java.sql.Types.BLOB);
                }
                statement.setInt(6, voziloId);

                statement.executeUpdate();
                connection.close();

                tout.setText("Avtomobil uspešno urejen!\n");
                ...
                res.setText("Avtomobil uspešno urejen!");
            ...
            new adminrent().setVisible(true);
            this.dispose();
        } else if (e.getSource() == naloziteSlikoButton) {
            JFileChooser fileChooser = new JFileChooser();
            int response = fileChooser.showOpenDialog(null);

            if (response == JFileChooser.APPROVE_OPTION) {
                selectedFile = fileChooser.getSelectedFile();
                resadd.setText("Pot do izbrane slike: " + selectedFile.getAbsolutePath());
            }
        }
    }
